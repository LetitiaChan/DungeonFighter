...
public class NcEffectBehaviour_ : MonoBehaviour
{
    ...
    {
        int nCount = 0;
        if (GetComponent<NcSpriteAnimation_>() != null)
            nCount++;
        if (GetComponent<NcUvAnimation_>() != null)
            nCount++;
        if (GetComponent<NcTilingTexture_>() != null)
            nCount++;
        if (GetComponent<NcSpriteTexture_>() != null)
            nCount++;
        return nCount;
    }
#endif
    ...
    static Texture[] PreloadTexture(GameObject tarObj, List<GameObject> parentPrefabList)
    {
        ...
        // prefab
        NcAttachPrefab_[] prefabs = tarObj.GetComponentsInChildren<NcAttachPrefab_>(true);
        foreach (NcAttachPrefab_ obj in prefabs)
        ...
        NcParticleSystem_[] pss = tarObj.GetComponentsInChildren<NcParticleSystem_>(true);
        foreach (NcParticleSystem_ ps in pss)
        ...
        NcSpriteTexture_[] sts = tarObj.GetComponentsInChildren<NcSpriteTexture_>(true);
        foreach (NcSpriteTexture_ st in sts)
        ...
        // sound
        NcAttachSound_[] ass = tarObj.GetComponentsInChildren<NcAttachSound_>(true);
        foreach (NcAttachSound_ ncas in ass)
            if (ncas.m_AudioClip != null)
                continue;

        // prefab & sound
        NcSpriteFactory_[] sprites = tarObj.GetComponentsInChildren<NcSpriteFactory_>(true);
        foreach (NcSpriteFactory_ sp in sprites)
    ...
    {
        NcEffectBehaviour_[] coms = target.GetComponentsInChildren<NcEffectBehaviour_>(true);
        foreach (NcEffectBehaviour_ com in coms)
    ...
    {
        NcParticleSystem_[] ncpss = gameObject.GetComponentsInChildren<NcParticleSystem_>(true);
        foreach (NcParticleSystem_ ncps in ncpss)
            if (ncps != null)
                ncps.SetDisableEmit();
        NcAttachPrefab_[] ncaps = gameObject.GetComponentsInChildren<NcAttachPrefab_>(true);
        foreach (NcAttachPrefab_ ncap in ncaps)
            if (ncap != null)
                ncap.enabled = false;
    ...
    protected GameObject CreateEditorGameObject(GameObject srcGameObj)
    {
        ...
        if (fxmMain != null && (this is NcDuplicator_) == false)
        {
    ...
    protected void ChangeParent(Transform newParent, Transform child, bool bKeepingLocalTransform, Transform addTransform)
    {
        ...
        if (bKeepingLocalTransform)
        {
            ...
            NcBillboard_[] ncBills = gameObject.GetComponentsInChildren<NcBillboard_>();
            foreach (NcBillboard_ ncb in ncBills)
                ncb.UpdateBillboard();
        }
    }
